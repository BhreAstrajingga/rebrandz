...
### Slugs
- Always provide a stable `slug` attribute for models that are exposed via URLs or used for human-friendly identifiers (e.g., `Post`, `Service`, `ServicePlan`, `Company` when applicable).
- The `User` model must also have a unique `slug` that is globally unique within the application.
- Prefer generating the slug from a primary human-readable attribute (usually `title` or `name`) using `Str::slug()` and ensure uniqueness with an incremental suffix.
